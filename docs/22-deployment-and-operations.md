...
- **Most PDFs no longer touch the ML stack, which changes the worker's cost profile.** `infrastructure/parsing/fast_pdf.py` reads a PDF's embedded text layer (pypdfium2/PDFium) and only hands *scanned* documents to docling. Measured on a one-page PDF, warm: 14,789.7ms via docling versus 10.9ms via the text layer. Two operational consequences: worker CPU for a text-native corpus is now negligible, and the docling/TorchInductor concerns above (`TORCH_COMPILE_DISABLE`, the baked-in Hugging Face models) apply only to the scanned-document path — they are still required, because that path is still reachable and still the only thing that reads a scan.
