...
- `extra="forbid"` on the root model — protects programmatic construction (e.g. a test passing an unexpected kwarg). The claim that pydantic-settings "was never going to see (or care about) `PATH` or other unrelated env vars" holds for ambient `os.environ`, but **not** for the dotenv file: `env_file=".env"` makes pydantic-settings enumerate every line in that file as an explicit input, so a name it can't place is rejected regardless of whether anything real reads it. That distinction mattered once `.env` also had to carry `docker compose`'s own `${VAR}` substitution names (`docker-compose.dev.yml`/`docker-compose.prod.yml` read the same file) — those are never a field under any spelling, and `extra="forbid"` refused to construct `Settings()` at all outside a container as a result. `settings_customise_sources` (`core/config.py`) now wraps the dotenv source and drops exactly two shapes of key before validation ever sees them: a fixed, named allowlist of compose-only names (`Settings._COMPOSE_ONLY_KEYS`), and a flat spelling of a nested group (`jwt_issuer` beside `jwt__issuer`) — safe to drop unconditionally there because `_reject_near_miss_group_names` has already run and would have raised if the correctly-spelled group were *not* also present. `extra="forbid"` still catches everything else, including a genuine typo of a top-level scalar field.
